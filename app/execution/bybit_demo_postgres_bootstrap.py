...
_CONFIRMATION_PHRASE = "APPLY_BYBIT_DEMO_DURABLE_SCHEMA"
_ADVISORY_LOCK_KEY = 119999
...
_SESSION_START_AUDIT_RELATION = "astra_bybit_demo_session_start_event_v123"
_SESSION_START_AUDIT_TRIGGERS = (
...
_MIGRATIONS = (
    ...
    ("v123", Path("migrations/v123/001_bybit_demo_session_start_audit.sql")),
)
...
def verify_bybit_demo_postgres_schema(dsn: str) -> BybitDemoPostgresBootstrapResult:
    """Verify the complete durable Demo schema without modifying PostgreSQL."""
    ...
    audit_relation, audit_triggers = _verify_session_start_audit_schema(dsn)
    relations_ready = (
...
def apply_bybit_demo_postgres_bootstrap(
    ...
    *,
    ...
    """Apply the complete durable Demo migration set under one stable advisory lock."""
...
def _verify_session_start_audit_schema(dsn: str) -> tuple[bool, bool]:
    if psycopg is None:
        raise RuntimeError("PostgreSQL dependency is unavailable")
    with psycopg.connect(dsn, autocommit=False) as connection:
        with connection.transaction():
            with connection.cursor() as cursor:
                cursor.execute("SET TRANSACTION READ ONLY")
                cursor.execute("SELECT to_regclass(%s)", (_SESSION_START_AUDIT_RELATION,))
                relation = cursor.fetchone()
                relation_ready = relation is not None and relation[0] is not None
                if not relation_ready:
                    return False, False
                cursor.execute(
                    """SELECT count(*)
                       FROM pg_trigger
                       WHERE NOT tgisinternal AND tgname = ANY(%s)""",
                    (list(_SESSION_START_AUDIT_TRIGGERS),),
                )
                trigger = cursor.fetchone()
                trigger_ready = trigger is not None and int(trigger[0]) == len(
                    _SESSION_START_AUDIT_TRIGGERS
