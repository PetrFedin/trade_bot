...
class CryptoCurrentDerivativesContext:
    ...
    def open_interest_regime(self) -> str:
        delta = self.open_interest_delta
        if delta is None:
            return "OI_UNKNOWN"
        if delta > 0:
            return "OI_RISING"
    ...
    def crowding_regime(self) -> str:
        ratio = self.long_account_ratio
        if ratio is None:
            return "CROWDING_UNKNOWN"
        if ratio >= _LONG_HEAVY:
            return "LONG_HEAVY"
        if ratio <= _SHORT_HEAVY:
            return "SHORT_HEAVY"
        return "BALANCED"

    @property
    def prior_funding_regime(self) -> str:
        rate = self.prior_funding_rate
        if rate is None:
    ...
    def validate(self) -> None:
        ...
        if self.open_interest_delta_fraction is not None:
            ...
            if self.open_interest_delta_fraction != (
                self.open_interest_delta / self.previous_open_interest
            ):
                raise ValueError("live crypto derivatives OI fraction is inconsistent")
        if self.long_account_ratio is not None and not _ZERO <= self.long_account_ratio <= _ONE:
            raise ValueError("live crypto derivatives long account ratio must be within [0, 1]")
        if self.short_account_ratio is not None and not _ZERO <= self.short_account_ratio <= _ONE:
            raise ValueError("live crypto derivatives short account ratio must be within [0, 1]")
...
class CryptoLiveOpportunity:
    ...
    def validate(self) -> None:
        ...
        if not self.market_universe_score.is_finite():
            raise ValueError("live crypto opportunity market score must be finite")
        ...
        if self.current_stress_score is not None and not 0 <= self.current_stress_score <= 5:
...
class CryptoLiveOpportunitySnapshot:
    ...
    def validate(self) -> None:
        ...
        if len(self.market_snapshot_id) != 64 or len(self.evidence_snapshot_id) != 64:
            raise ValueError("live opportunity snapshot source ids must be SHA-256 hex")
        ...
        if self.qualified_positive_count != sum(
        ...
        if self.qualified_mixed_count != sum(
...
def build_crypto_live_opportunity_snapshot(
    ...
    *,
    ...
    evidence_policy: CryptoStrategyEvidencePolicy | None = None,
) -> CryptoLiveOpportunitySnapshot:
    """Rank current fixed-strategy opportunities against the last qualified evidence matrix."""
    ...
    config.validate()
    policy = CryptoStrategyEvidencePolicy() if evidence_policy is None else evidence_policy
    policy.validate()
    ...
    preliminary: list[CryptoLiveOpportunity] = []
...
def _build_candidate(
    market_candidate: BybitOpportunityCandidate,
    *,
    ...
    evaluation = evaluate_crypto_signal(tuple(bars), config)
    ...
    if cell is None or not _required_bool(cell, "sample_sufficient"):
        reasons = (
            "NO_EXACT_HISTORICAL_CELL",
            if cell is None
            else "HISTORICAL_CELL_SAMPLE_INSUFFICIENT",
        )
        return _full_current_opportunity(
            market_candidate,
            signal,
...
def _signal_opportunity(
    ...
    *,
    ...
    base = _empty_opportunity(market, state=state, reasons=reasons)
    return CryptoLiveOpportunity(
        **{
            **base.__dict__,
...
def _signal_plan_opportunity(
    ...
    *,
    ...
    base = _signal_opportunity(market, signal, state=state, reasons=reasons)
    return CryptoLiveOpportunity(
        **{
            **base.__dict__,
    ...
    return CryptoLiveOpportunity(
        **{
            **base.__dict__,
            ...
            "evidence_total_net_pnl_usdt": _optional_decimal(
                cell, "total_net_pnl_usdt"
            ),
            "evidence_average_net_pnl_usdt": _optional_decimal(
                cell, "average_net_pnl_usdt"
            ),
            ...
            "evidence_drawdown_usdt": _optional_decimal(
                cell, "maximum_trade_sequence_drawdown_usdt"
            ),
...
def _with_rank(item: CryptoLiveOpportunity, *, rank: int) -> CryptoLiveOpportunity:
    payload = {**item.__dict__, "evidence_rank": rank}
    ranked = CryptoLiveOpportunity(**payload)
    ranked.validate()
    return ranked
