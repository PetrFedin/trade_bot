...
class PaperTradeUpdateProcessor:
    ...
    A stream-level duplicate is not, by itself, authority to mutate accounting. Exact
    fill replays are admitted only when this processor has previously observed the same
    digest pass stream validation and parser-agreement checks. This prevents a rejected
    or quarantined raw frame from acquiring accounting authority merely because the
    stream implementation remembers its digest.
    """
