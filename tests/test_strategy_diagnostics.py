...
def test_closed_form_baseline_flags_a_sub_baseline_record() -> None:
    """For a policy whose barriers never move, 137/608 sits far below the 33.3% flip.

    This exercises the closed-form path only. The shipped policy carries a trailing
    stop, so this baseline does not describe it; see the BASELINE_UNDETERMINED tests.
    """
    report = diagnostics.diagnose(evidence(137, 471), GEOMETRY, trailing_active=False)
...
def test_a_result_at_baseline_is_not_an_edge() -> None:
    ...
    report = diagnostics.diagnose(evidence(200, 400), GEOMETRY, trailing_active=False)
...
def test_a_real_edge_is_recognised() -> None:
    ...
    report = diagnostics.diagnose(evidence(280, 320), GEOMETRY, trailing_active=False)
    assert report["verdict"] == diagnostics.Verdict.EDGE_PRESENT
...
def test_a_small_lucky_sample_is_not_an_edge() -> None:
    ...
    report = diagnostics.diagnose(evidence(6, 4), GEOMETRY, trailing_active=False)
    assert report["promotion_allowed"] is False
...
def test_breakeven_rate_accounts_for_costs() -> None:
    free = diagnostics.diagnose(evidence(137, 471), GEOMETRY, cost_fraction=NO_COST,
                              trailing_active=False)
    charged = diagnostics.diagnose(evidence(137, 471), GEOMETRY, trailing_active=False)
...
def test_empty_evidence_is_rejected() -> None:
    with pytest.raises(SystemExit, match="EVIDENCE_EMPTY"):
        diagnostics.diagnose(evidence(0, 0), GEOMETRY, trailing_active=False)
...
def test_invalid_geometry_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="stop_fraction"):
        diagnostics.diagnose(evidence(1, 1), bad, trailing_active=False)
...
def test_gate_passes_when_evidence_shows_an_edge(tmp_path: Path) -> None:
    ...
    assert diagnostics.main([str(path), "--require-edge", "--bar-sigma", "0.02"]) == 0
...
def test_overclaimed_promotion_is_blocked(
    ...
    assert diagnostics.main([str(path), "--enforce-declared-promotion", "--bar-sigma", "0.02"]) == 1
...
def test_absent_edge_alone_does_not_fail_enforcement(tmp_path: Path) -> None:
    ...
    assert diagnostics.main([str(path), "--enforce-declared-promotion", "--bar-sigma", "0.02"]) == 0
...
def test_trailing_policy_without_volatility_is_undetermined() -> None:
    """The closed form does not describe a moving protective level, so refuse a verdict.

    Comparing against a baseline that does not match the policy yields a confident
    answer in whichever direction the mismatch happens to point. Refusing is correct.
    """
    report = diagnostics.diagnose(evidence(137, 471), GEOMETRY, trailing_active=True)
    assert report["verdict"] == diagnostics.Verdict.BASELINE_UNDETERMINED
    assert report["promotion_allowed"] is False
    assert "trailing" in report["reason"] or "protective level" in report["reason"]


def test_shipped_policy_carries_a_trailing_stop() -> None:
    """If this ever becomes false, the closed-form baseline becomes valid again."""
    assert diagnostics.policy_has_trailing_stop() is True


def test_simulated_baseline_is_far_below_the_closed_form() -> None:
    """The trailing stop suppresses target-first well below stop/(stop+target).

    The closed form gives 33.3% for a 2%/4% pair. Simulating the shipped policy gives a
    materially lower rate, which is exactly why the two must never be interchanged.
    """
    simulated = diagnostics.simulate_baseline(0.014, episodes=4000)
    assert 0.10 < simulated < 0.28
    assert simulated < GEOMETRY.baseline_target_first - 0.05


def test_simulated_baseline_rises_with_volatility() -> None:
    """Wider bars reach the target more often; a baseline that ignored this would lie."""
    low = diagnostics.simulate_baseline(0.012, episodes=4000)
    high = diagnostics.simulate_baseline(0.030, episodes=4000)
    assert high > low


def test_simulated_baseline_is_deterministic() -> None:
    assert diagnostics.simulate_baseline(0.014, episodes=2000) == diagnostics.simulate_baseline(
        0.014, episodes=2000
    )


def test_simulated_baseline_rejects_impossible_volatility() -> None:
    with pytest.raises(ValueError, match="bar_sigma"):
        diagnostics.simulate_baseline(0.0, episodes=100)


def test_committed_evidence_is_undetermined_without_volatility() -> None:
    """Today the repository cannot state a verdict on its own frozen record."""
