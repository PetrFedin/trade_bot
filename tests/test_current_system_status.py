...
def test_current_system_status_is_current_fail_closed_and_not_profitable() -> None:
    ...
    assert main["last_qualified_sha"] == "043fc3003b055dc1e953854798627048d3f26960"
    ...
    assert main["qualification"]["canonical_security_regression_run_id"] == 34039454929
    assert main["qualification"]["release_provenance_run_id"] == 34039454985
...
def test_c2a3_remains_qualified_strategy_free_v120_typed_persistence() -> None:
    status = load_status()
...
def test_c2a4_is_current_qualified_v119_excursion_persistence() -> None:
    status = load_status()
    ...
    assert c2a4["neutral_immutable_record_contract"] is True
    assert c2a4["deterministic_revision_sha256"] is True
    assert c2a4["runtime_table_privileges"] == ["DELETE", "INSERT", "SELECT", "UPDATE"]
    assert c2a4["runtime_migration_authority_allowed"] is False
    assert c2a4["postgresql_16_proven"] is True
    ...
    assert c2a4["focused_security_regression_result"] == "282 passed"
    assert c2a4["full_regression_result"] == (
        "1124 passed, 2 dedicated fleet-deployment skips"
    )
    assert c2a4["runtime_truncate_alter_drop_rejected"] is True
...
def test_current_consolidation_gate_is_c2b0_and_remains_fail_closed() -> None:
    ...
    assert consolidation["next_gate"] == (
        "C2B0_STRATEGY_FREE_V121_ARM_HALT_JOURNAL_AND_FAIL_CLOSED_CONTROL_STATE"
    )
    assert consolidation["next_gate_issue"] == 128
    assert consolidation["completed_database_security_issue"] == 107

    candidate = status["c2b0_candidate"]
    assert candidate["status"] == "PLANNED_AUDIT_IN_PROGRESS"
    assert candidate["tracking_issue"] == 128
    assert candidate["primary_source_pull_request"] == 80
    assert SHA40.fullmatch(candidate["primary_source_sha"])
    assert candidate["hardening_source_pull_request"] == 88
    assert SHA40.fullmatch(candidate["hardening_source_sha"])
    assert candidate["frozen_v121_001_git_blob"] == (
    ...
    assert candidate["automatic_arm_allowed"] is False
    ...
    assert blockers["P1-C2B0-V121-CONTROL-STATE"]["tracking_issue"] == 128
