...
def _seed(signal: datetime) -> None:
    ...
    with psycopg.connect(_DSN, autocommit=True) as connection:
        ...
        connection.execute(
            ...
            (snapshot_id, evidence_rank, symbol, market_rank, is_top10,
             signal_side, decision_time, signal_quality_score, universe_score,
             current_market_regime, current_open_interest_regime,
             current_crowding_regime, current_prior_funding_regime,
             current_stress_regime, current_stress_score, evidence_cell_key,
             ...
             evidence_drawdown_usdt, evidence_expected_net_edge_bps,
             evidence_expected_net_edge_to_cost, evidence_cost_to_risk,
             positive_historical_evidence, qualification_state,
             qualification_reason, planned_notional_usdt, risk_budget_usdt,
             target_net_profit_usd, estimated_round_trip_cost_usdt,
             estimated_round_trip_cost_bps, plan_reward_risk,
             plan_risk_budget_fraction, planned_notional_fraction_of_equity,
             operator_review_required, trade_actionable,
             strategy_parameters_changed, strategy_promotion_allowed,
             demo_activation_allowed, live_activation_allowed,
             bybit_live_order_routing_allowed)
            VALUES (%s, 1, 'BTCUSDT', 1, true, 'LONG', %s, 0.9, 0.95,
                    'BULL', 'OI_RISING', 'BALANCED', 'FUNDING_POSITIVE',
                    'STRESS_NORMAL', 1, %s, 40, true, 1.4, 0.6,
                    20, 0.5, 1.2, -0.6, 5, 30, 2, 0.2,
                    true, 'QUALIFIED_POSITIVE_EVIDENCE', 'EXACT_CELL_POSITIVE',
                    500, 5, 9, 1, 20, 2, 0.01, 0.5,
