...
@pytest.mark.parametrize(
    ...
    (
        (measured_context(adtv=Decimal("500")), "LIQUIDITY_PARTICIPATION_EXCEEDED"),
        (measured_context(equity=Decimal("200")), "POSITION_CONCENTRATION_EXCEEDED"),
        (
            measured_context(equity=Decimal("200"), sector_notional=Decimal("0")),
            "SECTOR_CONCENTRATION_EXCEEDED",
        ),
        (measured_context(volatility=Decimal("3")), "VOLATILITY_LIMIT_EXCEEDED"),
    ),
)
def test_complete_operational_observations_activate_configured_controls(
    tmp_path,
    context: OperationalRiskContext,
    expected_reason: str,
) -> None:
    runtime = build_local_product(config=config(), state_directory=tmp_path)
    _, intent, decision = runtime.paper_pipeline.plan(
