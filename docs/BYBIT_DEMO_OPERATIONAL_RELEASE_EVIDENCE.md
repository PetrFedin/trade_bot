...
7. `RECOVERY_DRILL_PROVEN` — a newly created controlled v123 `RECOVERED` receipt is bound to that exact HALT event.
...
Every producer artifact is bound to `GITHUB_SHA`. Readiness also carries a canonical `manifest_sha256`. ARM and HALT receipts are immutable control evidence and expose no order-submission capability. Entry is bound to the exact ARM event; recovery is bound to the exact HALT event.
