...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise StatusContractError(f"cannot load {relative}: {exc}") from exc
...
def _workflow_summary(qualification: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in required_green:
        if not _required_bool(github.get(key), f"github.{key}"):
            raise StatusContractError(
                f"qualified engineering manifest requires {key}=true"
            )
    return {
...
def build_tracked_status() -> dict[str, Any]:
    ...
    if live.get("status") != "SOURCE_AND_CI_QUALIFICATION_ONLY":
        raise StatusContractError(
            "V109 live status must remain source/CI qualification only"
        )
    ...
    if strategy.get("strategy_status") != "PROFITABILITY_NOT_PROVEN":
        raise StatusContractError(
            "strategy evidence must remain PROFITABILITY_NOT_PROVEN"
        )
    ...
    if not isinstance(capabilities, list) or not all(
        ...
        raise StatusContractError(
            "qualified_capabilities must be a non-empty string array"
        )
...
def _readme_block(status: dict[str, Any]) -> str:
    ...
    qualified_sha = repository["latest_engineering_qualified_main_sha"]
    scope = repository["engineering_qualification_scope"]
    workflows = engineering["workflow_run_count"]
    checks = engineering["check_run_count"]
    families = ", ".join(gate["required_families"])
    lines = (
        README_BEGIN,
        ...
        f"- Latest engineering-qualified `main`: `{qualified_sha}` (`{scope}`).",
        (
            f"- Post-merge engineering evidence: {workflows}/{workflows} workflow runs "
            f"and {checks}/{checks} check-runs successful."
        ),
        ...
        f"- Next vertical gate: issue #{gate['issue']} — `{gate['name']}` ({families}).",
        (
...
def _replace_readme_block(readme: str, block: str) -> str:
    if readme.count(README_BEGIN) != 1 or readme.count(README_END) != 1:
        raise StatusContractError(
            "README must contain exactly one generated status block"
        )
...
def _checkout_sha() -> str:
    ...
    try:
        ...
        raise StatusContractError(
            "cannot resolve checkout SHA from GITHUB_SHA or git"
        ) from exc
...
def _checkout_relation(observed_sha: str, qualified_sha: str) -> str:
    ...
    try:
        ...
        raise StatusContractError(
            "cannot compare checkout with qualified SHA"
        ) from exc
    ...
    raise StatusContractError(
        "git merge-base could not compare qualification ancestry"
    )
...
def render_runtime_status(
    ...
    *,
    ...
    if _SHA_RE.fullmatch(observed_sha) is None:
        raise StatusContractError(
            "observed_sha must be a 40-char lowercase SHA"
        )
...
def check() -> None:
    ...
    if actual_status != expected_status:
        raise StatusContractError(
            "STATUS_ARTIFACT_STALE: CURRENT_SYSTEM_STATUS.json"
        )
    ...
    if readme != expected_readme:
        raise StatusContractError(
            "STATUS_ARTIFACT_STALE: README generated block"
        )
...
def show() -> None:
    ...
    runtime = render_runtime_status(
        tracked,
        observed_sha=observed,
        relation=relation,
    )
    print(_json_text(runtime), end="")


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compile and validate ASTRA system status"
    )
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument(
        "--check",
        action="store_true",
        help="fail if tracked artifacts drift",
    )
    mode.add_argument(
        "--write",
        action="store_true",
        help="rewrite tracked generated artifacts",
    )
    mode.add_argument(
        "--show",
        action="store_true",
        help="print runtime status with exact SHA",
    )
