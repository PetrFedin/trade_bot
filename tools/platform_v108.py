...
def verify_release(root: Path) -> dict[str, object]:
    ...
    if not identity_path.is_file():
        return {"schema": 108, "status": "FAIL", "files_verified": 0, "findings": ["missing:RELEASE_IDENTITY_V108.json"]}
    try:
        identity = json.loads(identity_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError:
        return {"schema": 108, "status": "FAIL", "files_verified": 0, "findings": ["invalid:RELEASE_IDENTITY_V108.json"]}
    findings: list[str] = []
