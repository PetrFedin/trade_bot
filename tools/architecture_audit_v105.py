...
def audit(root: Path) -> dict[str, object]:
    ...
    findings.extend(stable_identity_findings(pyproject, minimum_version=(7, 35, 0)))
    ...
    postgres = (root / "app/runtime/postgres_fleet_operations_v105.py").read_text(
        encoding="utf-8"
    )
    ...
    migration = (root / "migrations/v105/001_production_worker_fleet_operations.sql").read_text(
        encoding="utf-8"
    )
    for token in (
        "SECURITY DEFINER",
