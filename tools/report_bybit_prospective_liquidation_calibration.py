from __future__ import annotations

import argparse
import json
import os
from datetime import UTC, datetime, timedelta
from pathlib import Path

from app.strategy.crypto_prospective_liquidation_calibration import (
    CryptoLiquidationCalibrationPolicy,
    diagnose_crypto_prospective_liquidation_calibration,
)
from app.strategy.crypto_prospective_liquidation_calibration_postgres import (
    PostgresCryptoProspectiveLiquidationCalibrationReader,
)

_DEFAULT_DSN_ENV = "BYBIT_OPPORTUNITY_DATABASE_DSN"


def build_liquidation_calibration_report(
    *,
    dsn: str,
    rolling_days: int | None = None,
    maximum_final_seeds: int = 100_000,
) -> dict[str, object]:
    if not dsn.strip():
        raise ValueError("liquidation calibration database DSN is required")
    if rolling_days is not None and not 1 <= rolling_days <= 3650:
        raise ValueError("liquidation calibration rolling days must be within [1, 3650]")
    now = datetime.now(UTC)
    start = None if rolling_days is None else now - timedelta(days=rolling_days)
    reader = PostgresCryptoProspectiveLiquidationCalibrationReader(dsn)
    dataset = reader.load_dataset(
        signal_available_at_or_after=start,
        maximum_final_seeds=maximum_final_seeds,
    )
    report = diagnose_crypto_prospective_liquidation_calibration(
        dataset,
        policy=CryptoLiquidationCalibrationPolicy(),
    )
    report["report_generated_at"] = now.isoformat()
    report["report_window"] = (
        "ALL_AVAILABLE_FORWARD_LIQUIDATION_HISTORY"
        if rolling_days is None
        else f"ROLLING_{rolling_days}_DAYS"
    )
    report["signal_available_at_or_after"] = None if start is None else start.isoformat()
    return report


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Report out-of-sample liquidation-context calibration from final prospective "
            "outcomes. This command is PostgreSQL read-only and cannot change ranking."
        )
    )
    parser.add_argument("--rolling-days", type=int)
    parser.add_argument("--maximum-final-seeds", type=int, default=100_000)
    parser.add_argument("--output")
    parser.add_argument("--database-dsn-env", default=_DEFAULT_DSN_ENV)
    return parser.parse_args()


def main() -> None:
    args = _parse_args()
    dsn = os.environ.get(args.database_dsn_env, "")
    if not dsn.strip():
        raise SystemExit(
            "required PostgreSQL DSN environment variable is missing:"
            + args.database_dsn_env
        )
    report = build_liquidation_calibration_report(
        dsn=dsn,
        rolling_days=args.rolling_days,
        maximum_final_seeds=args.maximum_final_seeds,
    )
    encoded = json.dumps(report, indent=2, sort_keys=True)
    if args.output:
        destination = Path(args.output)
        destination.parent.mkdir(parents=True, exist_ok=True)
        temporary = destination.with_suffix(destination.suffix + ".tmp")
        temporary.write_text(encoded + "\n", encoding="utf-8")
        temporary.replace(destination)
    print(encoded)


if __name__ == "__main__":
    main()
