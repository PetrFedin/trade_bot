...
def run_rest_signal_outcome_audit(
    *,
    host: str = "api.bybit.eu",
    ...
    end_ms = last_completed_kline_end_ms(now_ms=clock_ms, interval="5")
    interval_ms = interval_milliseconds("5")
    ...
    acquisition = BybitPublicKlineClient(host=host).fetch(request)
    ...
    if not isinstance(variants, dict) or len(variants) != 1:
        raise ValueError("REST signal audit expected exactly one replay target variant")
    variant = next(iter(variants.values()))
    if not isinstance(variant, dict):
        raise ValueError("REST signal audit replay target variant is invalid")
    ...
    audit = audit_crypto_signal_outcomes(
        records,
        strategy_config=config,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit frozen Bybit signals from regional public V5 klines"
    )
    parser.add_argument("--host", default="api.bybit.eu")
...
def main() -> None:
    ...
    report = run_rest_signal_outcome_audit(
        host=args.host,
        symbols=symbols,
