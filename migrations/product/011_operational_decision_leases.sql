...
CREATE TABLE IF NOT EXISTS astra_operational_decision_safety_evidence (
    ...
    last_bar_id TEXT
        REFERENCES astra_operational_market_bars(bar_id),
    bar_ids JSONB NOT NULL CHECK (jsonb_typeof(bar_ids) = 'array'),
    continuity_reasons JSONB NOT NULL CHECK (jsonb_typeof(continuity_reasons) = 'array'),
    readiness_reasons JSONB NOT NULL CHECK (jsonb_typeof(readiness_reasons) = 'array'),
...
ALTER TABLE astra_operational_decision_safety_evidence
ADD COLUMN IF NOT EXISTS bar_ids JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE astra_operational_decision_safety_evidence
ALTER COLUMN bar_ids DROP DEFAULT;
